...
export function playLose() {
  const audio = ctx();
  if (!audio) return;
  noiseHit(audio, { at: 0, dur: 0.45, vol: 0.22, freq: 90, type: "lowpass", q: 0.7, slide: -40 });
...
/** Short lime ping — a perfect is obvious even if you never hear it. */
export function playPerfect() {
  const audio = ctx();
  if (!audio) return;
  beep(audio, { type: "triangle", freq: 988, at: 0, dur: 0.07, vol: 0.055 });
  beep(audio, { type: "sine", freq: 1480, at: 0.03, dur: 0.11, vol: 0.045 });
  beep(audio, { type: "sine", freq: 1976, at: 0.06, dur: 0.09, vol: 0.03 });
}

/** Luxury perfect: cold glass, not a cheer. 0 homes. */
export function playGlassChime() {
  const audio = ctx();
  if (!audio) return;
  beep(audio, { type: "sine", freq: 1568, at: 0, dur: 0.42, vol: 0.045 });
  beep(audio, { type: "sine", freq: 2093, at: 0.03, dur: 0.5, vol: 0.035 });
  beep(audio, { type: "triangle", freq: 3136, at: 0.05, dur: 0.55, vol: 0.022, slide: -80 });
  beep(audio, { type: "sine", freq: 1174, at: 0.08, dur: 0.38, vol: 0.02, slide: -30 });
}
...
export function spawnBurst(particles, W, H, x, y, n = 28) {
  ...
  while (particles.length > 24) particles.shift();
}

/** Dust puff at a chop cut plane. */
export function spawnDust(particles, x, y, n = 12) {
  const palette = ["#c4b8a4", "#8a8478", "#d8d0c4", "#6e6a64", "#b0a898"];
  for (let i = 0; i < n; i++) {
    particles.push({
